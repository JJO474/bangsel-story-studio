...
export default function Studio({scenes:_scenes}:{scenes?:unknown}={}){
 ...
 async function analyze(){if(!videoFile)return;setAnalyzing(true);setAnalysisError("");try{const media=await extractFrames(videoFile),r=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...media,fileName:videoName})}),data=await r.json() as {error?:string;analysis?:Analysis};if(!r.ok)throw new Error(data.error||"분석에 실패했습니다.");if(!data.analysis)throw new Error("분석 결과가 비어 있습니다.");setAnalysis(data.analysis);setLines(data.analysis.cuts.map((x:AnalysisCut)=>x.dialogue));note("원본 중심 4컷 분석이 완료됐습니다.")}catch(e){setAnalysisError(e instanceof Error?e.message:"분석에 실패했습니다.")}finally{setAnalyzing(false)}}
